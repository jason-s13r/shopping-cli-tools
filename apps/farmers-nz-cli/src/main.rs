//! `farmers` -- Farmers New Zealand from the command line.
//!
//! The interesting code is in `packages/`: the storefront protocol in
//! `farmers-api`, the rendering in `cli-kit`. What is left here is the part that
...
fn main() -> ExitCode {
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("farmers: could not start the async runtime: {e}");
    ...
    match runtime.block_on(run(cli)) {
        ...
        Err(e) => {
            ...
            // thing would fix it. Only this binary knows it is called `farmers`,
            // so turning that into a command line happens here.
            if let Some(advice) = cli::advice(&e) {
                eprintln!("      {advice}");
            }
            // 2 misuse, 3 auth, 5 no such thing, 8 refused by the bot
            // manager -- so a script can tell them apart without reading this
            // text.
