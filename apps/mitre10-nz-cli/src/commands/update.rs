...
pub async fn run(app: &App, version: Option<String>, check: bool, pre: bool) -> AppResult<()> {
    ...
    let mut source = build_kit::update::Source::new(stamp.repo, "mitre10-nz-cli", stamp.version)
    ...
    let wanted = match &version {
        Some(text) => {
            ...
            build_kit::update::find(&releases, &want).ok_or_else(|| {
                AppError::usage(format!("there is no mitre10-nz-cli release {want}"))
            })?
        }
            ...
            None => {
                ...
                emit(
                    ...
                    &Outcome {
                        ...
                        note: preview.map(|r| {
                            format!(
                                "{} is available as a pre-release: mitre10 update --pre-release",
                                r.version
                            )
                        }),
                    },
