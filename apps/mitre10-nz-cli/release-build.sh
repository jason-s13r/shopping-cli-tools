...
#
# SHA256SUMS must cover every platform -- `mitre10 update` refuses a download it
...
if [[ -n "${STAGING_ROOT:-}" && -d "$staged" ]]; then
  ...
  cargo build --release
  tar -czf "release/$DISPAT_PACKAGE-$DISPAT_NEW_VERSION-$os-$arch.tar.gz" -C target/release mitre10
fi
